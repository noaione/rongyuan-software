...
import { detectSubject } from '../DriverIO/usb/DetectSupportDevice'
...
import { kCompany, kSeverAddr } from '../../appConfig'
import { Not } from 'typeorm'
import { MusicLightConfig, supportDev } from '../../supportDev'
...
export const downloadAppUpgradeFile = async (progress: (p: number) => void): Promise<void> => {
  const zippath = await httpDownload('/downloadAppUpgradeFile', 'upgrade_dist.zip', {}, progress)
  ...
  zipEntries.forEach(v => {
    console.log('vvvv', v.entryName, v.rawEntryName)
    if (v.entryName === 'dist/' || v.entryName.includes('_MACOSX/') || v.entryName.includes('.dll')) {
      return
    }
    zip.extractEntryTo(/*entry name*/v.entryName, /*target path*/__dirname, /*maintainEntryPath*/false, /*overwrite*/true);
  })
...
export namespace webService {
  ...
  export const userLogin = async (email: string, password: string) => {
    ...
    if (cookie !== undefined)
      SessionUser.setSession(cookie[0], res.data.user.id)
  ...
  export const shareConfig = async (config: Config, describe: string) => {
    ...
    const res = await httpClient('/share_config', { config, describe })
  ...
  export const shareMacro = async (macro: Macro, describe: string) => {
    macro.isShared = true
    const res = await httpClient('/share_macro', { macro, describe })
    if (res.errCode !== 0) return res
  ...
  export const downloadSharedMacro = async (sharedConfig: SharedMacro) => {
    const macro = sharedConfig.macro
...
export namespace DBService {
    ...
    if (v?.type === '设备插入') {
      ...
      getCurrentUser().then(async (user) => {
        ...
        const res = user.deviceTypes.some(
          (devT) =>
            devT.pid === v.deviceType.pid && devT.vid === v.deviceType.vid
        )
  ...
  export const devToConfig = async () => {
  ...
  export const configToDev = async (config: Config) => {
    const dev = getCurrentDev()
    ...
    if (config.value !== undefined) await dev.setKeyConfig(config.value)

    if (dev.type === 'mouse') {
      if (config.dpi !== undefined) await dev.setDPI(config.dpi)
    } else {
      if (config.lightPic !== undefined && config.lightPic !== null) await dev.setLightPic(config.lightPic)
    }
  }
  ...
  export const getCurrentComanyDevList = async () => {
    return await DeviceType.find({ where: { company: kCompany, name: Not('help') } })
  }
