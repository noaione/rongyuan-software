...
export class ShareStore extends BaseStore {
    constructor() {
        super()
        setTimeout(() => {
            when(() => mobxStore.pageStore.pageIndex === 3, () => {
                this.autoRefresh()
            })
    ...
    @action.bound
    autoRefresh() {
        if (mobxStore.pageStore.pageIndex === 3 && mobxStore.deviceStore.currentDev !== undefined) {
            if (this.currentShareListType === 'config')
    ...
    downloadSharedConfig(sharedConfig: webService.SharedConfig) {
        if (mobxStore.userStore.user === undefined) {
            mobxStore.toastStore.setInfo('请先登录')
            return
        }
        ...
        for (let i = 0; i < config.value.length; i++) {
            ...
            if (cv.type === 'ConfigMacro') {
                if (mobxStore.deviceStore.currentDev?.checkMacroLenthIsFull(cv.macro)) {
                    mobxStore.toastStore.setInfo('宏过长,与当前设配不适配')
                    return
                }
            }
        }
    ...
    commonHandleSuccess = (v: any) => {
        ...
        if (ret.errCode !== 0 && ret.errMsg !== undefined) {
            ...
            mobxStore.toastStore.setErr(ret.errCode < 0 ? '网络连接失败' : ret.errMsg.toString())
