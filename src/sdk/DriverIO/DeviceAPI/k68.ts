...
export class K68 extends Dk2017 {
    ...
    setLightPic = async (pic: UserPicItem[]) => {
        ...
        const buf = Buffer.alloc(600).fill(0xff)
    ...
    // 只在驱动开始的时候getLightPic一次
    // 只获取了option 1 的LightPIc
    // 根据option的id获取响应的LightPic
    // how to get option's id !!!!!!!!!???????
    // 数据更新是代码不执行???????
    // deviceStore.getLightPic报错且执行
    getLightPic = async () => {
        // const { deviceStore } = useStore()
        // deviceStore.getLightPic()
        const b = Buffer.alloc(64)
        b[0] = 0x89
        b[1] = 0
