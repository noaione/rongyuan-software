...
const showTimeValue = (time: number) => {
  ...
  else return <TimeElement time={time / 1000} describe={'秒'} />
}
...
export const MacroButton = (p: {
  ...
  const editMoveFinished = (x: string, y: string) => {
    ...
    p.editHandle({
      ...p.item,
      dx: x !== '' ? parseInt(x) : p.item.dx,
      dy: y !== '' ? parseInt(y) : p.item.dy,
    })
    setEditable(false)
  }
  ...
  return (
    <Fragment>
      <Div
        ...
        type={p.isAddState ? 'HightLight' : 'Default'}>
