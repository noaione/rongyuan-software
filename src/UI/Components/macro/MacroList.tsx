...
export const MacroList = () => {
  ...
  const [headContextMenu, setHeadContextMenu] = useState(false)
  // document.onclick = ()=>{
  //   setHeadContextMenu(false)
  // }
  const changeHeadContextMenu = () => {
    macroStore.recodingState === 'recording'
      ? setHeadContextMenu(false)
      : setHeadContextMenu(!headContextMenu)
  }
    ...
    return (
      <Fragment>
        <dj.AddMacroContextMenu
          ...
          y={4}
          mode={'Head'}
          isOpen={headContextMenu && macroStore.recodingState === 'stop'}
          changeSub={()=>{
